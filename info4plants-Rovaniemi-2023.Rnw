...
\begin{frame}{\senseicon\ Sensing: one word and many meanings}
  \begin{itemize}
    \item Sensing of the state of environment.
    \item Sensing of the internal state of an organism.
    \item Sensing as detrimental to fitness or biomass production.
    \item Sensing as enhancer of fitness or biomass production.
  \end{itemize}
  Idea: \senseicon\ sensing is `exploration' and acquisition of information by an organism.
\end{frame}
...
\begin{frame}{\stressicon\ $\rightleftarrows$ \senseicon\ Stress and sensing are intertwined}
  ...
  \end{itemize}
  The outcome of sensing is acclimation, and has been selected through evolution and determines experienced stress.
\end{frame}
...
\begin{itemize}
  ...
  \item Memory is storage of information.
